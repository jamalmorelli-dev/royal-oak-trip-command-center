...
export default function CurrentTravelHUD({onSelectTab}){
  ...
  return <header className="hud-container" role="banner" aria-label="Live Travel Heads-Up Display">
    <div className="hud-credentials-bar">
      ...
      <div className="hud-actions no-print"><button type="button" className="hud-btn hud-btn-primary" onClick={()=>onSelectTab?.('Airport Docs')}>Airport Docs</button><button type="button" className="hud-btn hud-btn-secondary" onClick={()=>onSelectTab?.('Plane Checklist')}>Plane Checklist</button></div>
    </div>
